...
# =============================================================================
# 第一部分：SDK 自动加载器 (Auto SDK Loader)
# 作用：自动去海康安装目录寻找库文件，免去手动复制的麻烦
# =============================================================================
HAS_HIK_SDK = False
try:
    # 1. 定义可能的安装路径
    possible_sdk_paths = [
        os.path.join(os.getcwd(), "MvImport"), # 当前目录
    ...
    HAS_HIK_SDK = True
    print(f"[系统] 海康 SDK 加载成功 (路径: {p})")
except ImportError:
    print("[警告] 未找到海康 MVS SDK，SDK直连模式将不可用 (但不影响其他功能)")
    HAS_HIK_SDK = False

# =============================================================================
# 第二部分：海康相机驱动封装 (HikCamera Driver)
# =============================================================================
class HikCameraWrapper:
    ...
    def open_by_ip(self, ip):
        ...
        if ret != 0: raise Exception(f"枚举失败: {ret}")
...
# =============================================================================
# 第四部分：主程序界面 (Main GUI Application)
# =============================================================================
DEFAULT_CONFIG = {
...
class UniversalVisionServer:
    def __init__(self, root):
        self.root = root
        self.root.title("通用视觉服务器系统 - 终极单文件版 (All-in-One)")
    ...
    def setup_config_ui(self, p):
        ...
        tk.Radiobutton(mf, text="海康 SDK (推荐)", variable=self.var_camera_mode, value="sdk").pack(side=tk.LEFT)
        ...
        if not HAS_HIK_SDK: tk.Label(f_right, text="(未找到海康SDK, SDK模式不可用)", fg="red").grid(row=r_row, column=1, sticky="w"); r_row+=1
    ...
    def test_camera(self):
        ...
        try:
            if mode == "sdk":
                ...
                if ret: messagebox.showinfo("成功", f"SDK连接成功 {f.shape}")
                else: raise Exception("读取失败")
            else:
    ...
    def cam_loop(self):
        while self.is_running:
            try:
                if self.cap:
                    if hasattr(self.cap, 'read'): ret, f = self.cap.read() # OpenCV or Wrapper
