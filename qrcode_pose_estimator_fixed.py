...
class QRCodePoseEstimator:
    def __init__(self, root):
        self.root = root
        self.root.title("二维码姿态解算工具（高精度版-Fixed）")
    ...
    def run_pose_estimation(self):
        ...
        self.draw_overlay(img, img_points_sub, rvec_refine, tvec_refine, 
                          (smooth_roll, smooth_pitch, smooth_yaw), distance, camera_matrix_scaled)
    ...
    def draw_overlay(self, img, pts, rvec, tvec, angles, distance, camera_matrix):
        """在图像上绘制二维码框、坐标轴、位姿信息"""
        ...
        # 使用传入的 camera_matrix 进行投影
        imgpts, _ = cv2.projectPoints(axis_pts, rvec, tvec, camera_matrix, self.dist_coeffs)
