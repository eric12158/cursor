...
try:
    # 1. 尝试加载 MVS Runtime DLL (解决 Python 3.8+ 找不到 DLL 的问题)
    possible_dll_paths = [
...
class HikCameraWrapper:
    ...
    def open_by_ip(self, ip):
        ...
        # 增加包间隔 (减少网络拥堵)
        self.handle.MV_CC_SetIntValue("GevSCPD", 1000)
        ...
        self.is_opened = True
        print(f"[SDK] 相机 {ip} 打开成功，包长: {nPacketSize}")
...
# =============================================================================
# 模块四：主程序界面与逻辑 (UniversalVisionServer)
# =============================================================================
...
class UniversalVisionServer:
    def __init__(self, root):
        self.root = root
        self.root.title("通用视觉服务器系统 - 终极完全体 (Ultra Edition)")
    ...
    def setup_run_ui(self, parent):
        ...
        # 1. 核心控制
        s_frame = tk.LabelFrame(left_frame, text="系统控制", font=("bold", 10))
        s_frame.pack(fill=tk.X, padx=5, pady=5)
        
        self.btn_start = tk.Button(s_frame, text="启动服务 (Start Server)", command=self.toggle_server, bg="#4caf50", fg="white", height=2, font=("微软雅黑", 11, "bold"))
        self.btn_start.pack(fill=tk.X, padx=5, pady=5)
        
        st_frame = tk.Frame(s_frame)
        st_frame.pack(fill=tk.X, padx=5, pady=2)
        self.lbl_status = tk.Label(st_frame, text="状态: 已停止", fg="red", font=("Arial", 10))
        self.lbl_status.pack(side=tk.LEFT)
        self.lbl_client = tk.Label(st_frame, text="客户端: 无连接", fg="gray", font=("Arial", 10))
        self.lbl_client.pack(side=tk.RIGHT)
        
        # 2. 手动调试区 (新增功能)
        m_frame = tk.LabelFrame(left_frame, text="手动调试 (Manual Debug)", font=("bold", 10), fg="blue")
        m_frame.pack(fill=tk.X, padx=5, pady=5)
        
        tk.Button(m_frame, text="📸 手动拍照 (Simulate Trigger)", command=self.manual_trigger, bg="#2196f3", fg="white", height=2).pack(fill=tk.X, padx=5, pady=5)
        tk.Label(m_frame, text="* 点击此按钮等同于机械臂发送了指令，\n可用于不连机械臂时的单独测试。", fg="gray", justify=tk.LEFT).pack(anchor="w", padx=5)
        ...
        tk.Checkbutton(a_frame, text="启用实时畸变矫正 (需内参)", variable=self.undistort_view).grid(row=1, column=0, sticky="w", columnspan=2)
    ...
    def setup_config_ui(self, parent):
        ...
        add_entry(f_left, "监听 IP:", "robot_net", "bind_ip")
        add_entry(f_left, "监听端口:", "robot_net", "port")
        
        tk.Label(f_left, text="--- 协议设置 ---", fg="gray").grid(row=row, column=0, columnspan=2, pady=10); row+=1
        add_entry(f_left, "触发指令:", "commands", "trigger")
        add_entry(f_left, "失败返回:", "commands", "error")
        add_entry(f_left, "成功前缀:", "commands", "success_prefix")
        
        tk.Label(f_left, text="--- 存储设置 ---", fg="gray").grid(row=row, column=0, columnspan=2, pady=10); row+=1
        ...
        tk.Label(f_right, text="驱动模式:").grid(row=r_row, column=0, sticky="e", padx=5, pady=5)
        ...
        if not HAS_HIK_SDK:
            tk.Label(f_right, text="[警告] SDK加载失败，请检查环境", fg="red").grid(row=r_row, column=1, sticky="w"); r_row+=1
        
        add_cam_entry("目标 IP:", "target_ip")
        ...
        tk.Label(f_right, text="--- Halcon 内参 ---", fg="gray").grid(row=r_row, column=0, columnspan=2, pady=5); r_row+=1
        add_cam_entry("Fx:", "fx")
        add_cam_entry("Fy:", "fy")
        add_cam_entry("Cx:", "cx")
        add_cam_entry("Cy:", "cy")
    ...
    def draw_placeholder(self):
        ...
        if w < 10: w=800; h=600
        self.canvas.create_text(w//2, h//2, text="等待视频源...\n(点击启动服务)", fill="gray", font=("Arial", 16), justify=tk.CENTER)

    def update_cfg_from_ui(self):
        # 这是一个通用的反射更新，将 var_xxx 的值写回 self.cfg
    ...
    def toggle_server(self):
        if not self.is_running:
            ...
            try:
                ...
                mode = self.cfg["camera"]["mode"]
                if mode == "sdk":
                    if not HAS_HIK_SDK: raise Exception("SDK 未加载")
    ...
    def update_disp(self, img):
        ...
        if self.show_focus_score.get():
            ...
            cv2.putText(display_img, f"Focus Score: {int(score)}", (20, 40), 
    ...
    def manual_trigger(self):
        """手动触发拍照逻辑"""
        if not self.is_running:
            messagebox.showwarning("提示", "请先点击'启动服务'，连接相机后再拍照。")
            return
        
        self.log("用户手动触发拍照...")
        res = self.process_request()
        self.log(f"手动处理结果: {res}")
        messagebox.showinfo("完成", f"拍照完成，结果已记录。\n返回: {res}")
    ...
    def run_hand_eye_calc(self):
        ...
        try:
            ...
            for d in valid:
                ...
                R_gripper2base.append(R.from_euler('xyz', pose[3:], degrees=True).as_matrix())
